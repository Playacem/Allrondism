...
public class TileEntityMultiFurnaceCore extends TileAM implements ISidedInventory {
    ...
    /**
     * Cook progress(the bar), if this reaches
     * {@link playacem.allrondism.tileentity.TileEntityMultiFurnaceCore#cookDuration
     * cookDuration} items will be smelt
     */
    ...
    public int getDirection() {
        ...
        int metadata = getBlockMetadata(); // worldObj.getBlockMetadata(xCoord,
                                           // yCoord, zCoord);
            ...
            for (int vert = negativeMax; vert <= max; vert++) {
                for (int depth = 0; depth <= maxDepth; depth++) {
                    ...
                    if (!(UtilBlock.isValidBlock(worldObj, x, y, z, ModBlocks.storageBlock.blockID, 1) || blockID == ModBlocks.multiFurnaceExtension.blockID)) {
                        StringBuilder sb = new StringBuilder();
                        sb.append(String.format("A block with id %d located relative to the Core at horiz: %d, vert: %d, depth: %d (x: %d, y: %d, z: %d) is not usable with this MultiBlock (Size: %d)!", blockID, horiz, vert, depth, x, y, z, size));
                        errorMsg = sb.toString();
                        LogHelper.debug(sb.toString());
                        return false;
                    }
    ...
    private void revertDummies(int size) {
        ...
        for (int horiz = negativeMax; horiz <= max; horiz++) { // Horizontal (X
                                                               // or Z)
            for (int vert = negativeMax; vert <= max; vert++) { // Vertical (Y)
                for (int depth = 0; depth <= maxDepth; depth++) { // Depth (Z or
                                                                  // X)
                    int x = xCoord + (forwardZ ? horiz : depth * depthMultiplier);
                    int y = yCoord + vert;
                    int z = zCoord + (forwardZ ? depth * depthMultiplier : horiz);
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (isBurning() && canSmelt()) {
                ...
                if (furnaceCookTime == cookDuration * (int) cookDurationFactor) {
                    furnaceCookTime = 0;
                    smeltItems();
                    inventoryChanged = true;
                }
    ...
    private int getSmeltingFlag() {
        ...
        for (int i = 0; i < inputSlots.length; i++) {
            if (inventory[inputSlots[i]] != null
                    && (inventory[outputSlots[i]] == null || inventory[outputSlots[i]].isItemEqual(FurnaceRecipes.smelting().getSmeltingResult(inventory[inputSlots[i]])))) {
                result += array[i];
            }
        }
